...
/*=======================================
 ...
 ========================================*/
static void rt595_m33_mmap_init(MachineState *machine)
{
    int i = 0;
    int j = 0;
    uint32_t base = 0x0;
    ...
    base = 0x100000;
    for(i = 16, j = 0; i < RT595_M33_SRAM_BANKS; i++, j++)
    {
    ...
    _memmap.flexspi1_ctl.start = 0x4013C000;
    _memmap.flexspi1_ctl.size = 0x1000;
    ...
    _memmap.flexcomm[8].start = 0x40209000;
    _memmap.flexcomm[8].size  = 0x1000;
    _memmap.flexcomm[8].irq_num   = 60;
    _memmap.flexcomm[9].start = 0x4020A000;
    _memmap.flexcomm[9].size  = 0x1000;
    _memmap.flexcomm[9].irq_num   = 61;
    _memmap.flexcomm[10].start = 0x4020B000;
    _memmap.flexcomm[10].size  = 0x1000;
    _memmap.flexcomm[10].irq_num   = 62;
    _memmap.flexcomm[11].start = 0x4020C000;
    _memmap.flexcomm[11].size  = 0x1000;
    _memmap.flexcomm[11].irq_num   = 63;
    _memmap.flexcomm[12].start = 0x4020D000;
    _memmap.flexcomm[12].size  = 0x1000;
    _memmap.flexcomm[12].irq_num   = 64;
    _memmap.flexcomm[13].start = 0x4020E000;
    _memmap.flexcomm[13].size  = 0x1000;
    _memmap.flexcomm[13].irq_num   = 65;
    ...
    _memmap.flexcomm[16].start = 0x40128000;
    _memmap.flexcomm[16].size  = 0x1000;
    _memmap.flexcomm[16].irq_num   = 66;
...
static void rt595_resolve_machine_flexspi_nor(RT595_M33_MachineState *mms)
{
    ...
    backend = (HostMemoryBackend *)object_resolve_path_type("rt595_flexspi_nor",
...
static I2CBus *rt595_i2c_get_bus(RT595_M33_MachineState *soc, uint32_t num)
{
    BusState * bus;
...
static void rt595_evk_i2c_init(RT595_M33_MachineState *soc)
{
    /* fxos8700  */
    i2c_slave_create_simple(rt595_i2c_get_bus(soc, 4), TYPE_FXOS8700, (0x1e<<1));
    /* pmic */
    i2c_slave_create_simple(rt595_i2c_get_bus(soc, 15), TYPE_PCA9420, (0x61<<1));
}
...
static void rt595_resolve_flexcomm_uart(RT595_M33_MachineState *mms, int flexcomm_num, int uart_id)
{
    char *flexcomm_name = g_strdup_printf("flexcomm%d_s", flexcomm_num);
    qemu_irq irq = rt595_irq_init(mms, _memmap.flexcomm[flexcomm_num].irq_num, 0);
...
static void rt595_resolve_flexcomm_i2c(RT595_M33_MachineState *mms, int flexcomm_num)
{
    char *flexcomm_name = g_strdup_printf("flexcomm%d_s", flexcomm_num);
    qemu_irq irq = rt595_irq_init(mms, _memmap.flexcomm[flexcomm_num].irq_num, 0);
...
static void rt595_resolve_lpadc(RT595_M33_MachineState *mms, int id)
{
    char *lpadc_name = g_strdup_printf("lpadc%d_s", id);
    qemu_irq irq = rt595_irq_init(mms, _memmap.lpadc.irq_num, 0);
...
static void rt595_resolve_ostimer(RT595_M33_MachineState *mms, int id)
{
    char *ostimer_name = g_strdup_printf("ostimer%d_s", id);
    qemu_irq irq = rt595_irq_init(mms, _memmap.ostimer[id].irq_num, 0);
...
static void rt595_m33_common_init(MachineState *machine)
{
    ...
    RT595_M33_MachineClass *mc = RT595_MACHINE_GET_CLASS(mms);
    ...
    //memory_region_init(system_memory, OBJECT(machine), "rt595 amba", UINT64_MAX);
    ...
    //qdev_prop_set_uint32(iotkitdev, "EXP_NUMIRQ", RT595_M33_NUMIRQ);
...
#endif
    int i = 0;
    for (i=0; i < RT595_M33_SRAM_BANKS; i++) {
    ...
    rt595_resolve_flexcomm_uart(mms, 0, 0); /* dbug uart */
    rt595_resolve_flexcomm_i2c(mms, 4);  /* i2c accelerator meter*/
    rt595_resolve_flexcomm_uart(mms, 12, 1); /* arduio uart*/
    rt595_resolve_flexcomm_i2c(mms, 15); /* pmic i2c */
...
static void rt595_m33_idau_check(IDAUInterface *ii, uint32_t address,
...
static Property rt595_m33_soc_properties[] = {
    DEFINE_PROP_UINT32("boot-base-addr", RT595_M33_MachineState, boot_base_addr,
                        0x18001000),
    DEFINE_PROP_END_OF_LIST(),
};

static void rt595_m33_class_init(ObjectClass *oc, void *data)
{
    IDAUInterfaceClass *iic = IDAU_INTERFACE_CLASS(oc);
    MachineClass *mc = MACHINE_CLASS(oc);
    RT595_M33_MachineClass *mmc = RT595_MACHINE_CLASS(oc);

    iic->check = rt595_m33_idau_check;
    ...
    mc->desc = "RT595 for Cortex-M33";
    mc->default_ram_size = 128 * MiB;
    mc->init = rt595_m33_common_init;
    mmc->armsse_type = TYPE_RT500_M33;
    device_class_set_props(DEVICE_CLASS(oc), rt595_m33_soc_properties);
}

static const TypeInfo rt595_m33_info = {
    .name = TYPE_RT595_M33_MACHINE,
    .parent = TYPE_MACHINE,
    .instance_size = sizeof(RT595_M33_MachineState),
    .class_size = sizeof(RT595_M33_MachineClass),
    .class_init = rt595_m33_class_init,
};

static void rt595_m33_machine_init(void)
{
    type_register_static(&rt595_m33_info);
}

type_init(rt595_m33_machine_init);
